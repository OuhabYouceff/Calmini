...
class EvenementC {
    ...
    function AjouterEvenement($Evenement){
       ...
       $sql = "INSERT INTO evenement  (	 Title , date_deb, date_fin, Adresse, Organisateurs,cout,Tags,Image) 
                 VALUES ( :Tilte, :date_deb , :date_fin, :Adresse, :organisateurs, :cout,:tags, :img)";
    $db = config::getConnexion();
    try{
...
function modifierevent($id,$Evenement){
       try{
        $db = config::getConnexion();
$query = $db->prepare('UPDATE evenement SET Id_event = :Id_event, Title = :Title, date_deb = :date_deb, date_fin = :date_fin, Adresse = :Adresse, Organisateurs = :org, cout = :cout, Tags = :tags ,Image = :img WHERE Id_event = :Id_event');
$query->execute([
            'Id_event'=> $id,
